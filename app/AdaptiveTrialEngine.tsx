...
export default function AdaptiveTrialEngine(){
  ...
  useEffect(()=>{
    async function adaptState(input?:AppState){
      ...
      const current=loadLocalState(userId)||input;
      if(!current)return;

      for(const path of current.paths){
        if(running.current.has(path.id))continue;
        ...
        // Keep first-run Forge quests intact. Adaptation begins after the user has
        // demonstrated at least one real result on the path.
        ...
        try{
          ...
          if(!res.ok||!data?.title||!data?.proof)continue;

          const latest=loadLocalState(userId)||current;
          const stillOpen=latest.quests.find(q=>q.id===nextOpen.id&&!q.done&&!q.aiGenerated);
          ...
          // The deterministic cycle engine already created a valid next Trial.
          // Adaptive AI enhances it but never blocks the user's progression.
          console.warn("BECOMR adaptive Trial fallback used",err);
        }finally{
          running.current.delete(path.id);
        }
      }
    }

    const onCycle=(event:Event)=>adaptState((event as CustomEvent<AppState>).detail);
    const initial=window.setTimeout(()=>adaptState(),1400);
