...
import { createStore, type StateStore } from "@/lib/storage";
import {
...
function DynamicInsignia({state,selected,onSelect,compact=false}:{state:AppState;selected:string;onSelect?:(id:string)=>void;compact?:boolean}){
  ...
  return <div className={`sig-wrap ${compact?"compact":""}`}>
    ...
    <svg viewBox="0 0 800 720" className="sig-svg">
      ...
      <g className="sig-trunk">
        <path d="M400 525 C375 470 386 421 394 376 C398 343 398 305 400 270 C402 305 402 343 406 376 C414 421 425 470 400 525Z"/>
      </g>
      {state.paths.map((p,i)=>{
        ...
        return <g key={p.id} className={`sig-path stage-${stage} ${sel?"selected":""}`} onClick={()=>onSelect?.(p.id)}>
          ...
          {ornate && <>
            <ellipse cx={end[0]} cy={end[1]} rx="14" ry="6" transform={`rotate(${i*38-120} ${end[0]} ${end[1]})`} className="sig-leaf"/>
          ...
          {!compact && <><text x={end[0]} y={end[1]+42} textAnchor="middle" className="path-name">{p.name.toUpperCase()}</text>
          <text x={end[0]} y={end[1]+55} textAnchor="middle" className="path-stage">{stage.toUpperCase()} · {progress}%</text></>}
        </g>
      })}
...
function QuestEditor({state,setState,onClose}:{state:AppState;setState:(s:AppState)=>void;onClose:()=>void}){
  ...
  return <div className="modal-backdrop"><section className="editor-sheet">
    <p className="kicker">PHASE 13 / QUEST EDITOR</p><h2>Create a new Trial.</h2>
    ...
    <label>TITLE<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="What should be proven?"/></label>
    <label>PROOF REQUIRED<textarea value={proof} onChange={e=>setProof(e.target.value)} placeholder="What result proves success?"/></label>
    <div className="editor-grid"><label>XP<input type="number" min="5" step="5" value={xp} onChange={e=>setXp(Number(e.target.value))}/></label>
    <label>TYPE<select value={kind} onChange={e=>setKind(e.target.value as Quest["kind"])}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="boss">Boss</option></select></label></div>
...
function PathEditor({state,setState,onClose}:{state:AppState;setState:(s:AppState)=>void;onClose:()=>void}){
  ...
  return <div className="modal-backdrop"><section className="editor-sheet">
    <p className="kicker">PHASE 13 / PATH EDITOR</p><h2>Open a new path.</h2>
    <label>PATH NAME<input value={name} onChange={e=>setName(e.target.value)} placeholder="Cooking, Architecture, Guitar..."/></label>
    <label>GLYPH<input value={glyph} onChange={e=>setGlyph(e.target.value)} maxLength={2}/></label>
    <label>CAPABILITY<input value={cap} onChange={e=>setCap(e.target.value)} placeholder="What will this path make you capable of?"/></label>
...
export default function Home(){
  ...
  useEffect(()=>{(async()=>{const s=await createStore();setStore(s);const loaded=await s.load();if(loaded)setState(loaded);setReady(true)})()},[]);
  useEffect(()=>{if(ready&&store)store.save(state).catch(console.error)},[state,ready,store]);
  ...
  function prove(q:Quest){
    ...
    setState({...state,
      xp:state.xp+(q.done?0:q.xp),
      quests:state.quests.map(x=>x.id===q.id?{...x,done:true,evidence:proof.trim(),completedAt:now}:x)
    });
  ...
  return <main className="shell">
    <div className="noise"/>
    <header className="masthead"><BrandMark/><div className="mast-meta"><span>LVL {level}</span><i/><span>{state.xp.toLocaleString()} XP</span><i/><span>☀ {state.momentum}</span></div></header>

    {tab==="command"&&<section className="page">
      <div className="command-heading"><div><p className="kicker">COMMAND / LIVE PROGRESSION</p><h1>Proof changes the <em>tree.</em></h1></div><div className="command-actions"><button onClick={()=>setModal("quest")}>+ QUEST</button><button onClick={()=>setModal("path")}>+ PATH</button></div></div>
      <div className="command-grid">
        <section><div className="section-rule"><span>01</span><b>OPEN TRIALS</b><i/></div>
          {dailies.map(q=>{const p=state.paths.find(x=>x.id===q.pathId);return <button key={q.id} className={`trial-row ${q.done?"done":""}`} onClick={()=>{if(!q.done){setActiveQuest(q);setProof(q.evidence||"")}}}>
            <span className="trial-glyph-sm">{p?.glyph||"✦"}</span><div><small>{p?.name.toUpperCase()} · {q.kind.toUpperCase()}</small><strong>{q.title}</strong><p>{q.proof}</p></div><em>{q.done?"PROVEN":`+${q.xp} XP`}</em>
          </button>})}
        </section>
        <aside><div className="section-rule"><span>02</span><b>LIVE INSIGNIA</b><i/></div><DynamicInsignia state={state} selected={selectedPath} compact/></aside>
      </div>
    </section>}

    {tab==="compass"&&<section className="page">
      <div className="compass-title"><div><p className="kicker">PHASE 12 / DATA-DRIVEN COMPASS</p><h1>The symbol only grows when <em>proof exists.</em></h1></div><p>Every branch now reads XP from completed quests. No path percentage is hardcoded.</p></div>
      <DynamicInsignia state={state} selected={selectedPath} onSelect={setSelectedPath}/>
      {selected&&<section className="path-inspector">
        <div className="inspector-intro"><span className="big-glyph">{selected.glyph}</span><div><p>{selected.region}</p><h2>{selected.name}</h2><span>{selected.capability}</span></div><div className="stage-readout"><small>EARNED XP</small><strong>{pathEarnedXp(state,selected.id)}</strong><em>{pathProgress(state,selected)}%</em></div></div>
    ...
    {tab==="weekly"&&<section className="page">
      <div className="compass-title"><div><p className="kicker">PHASE 14 / WEEKLY BOSSES</p><h1>Weeks need a <em>boss.</em></h1></div><div className="weekly-score"><strong>{wScore}%</strong><span>WEEK CLEARED</span></div></div>
      <div className="weekly-grid">
        ...
        <aside><div className="section-rule"><span>II</span><b>GENERATE FROM CURRENT PATH</b><i/></div>{state.paths.slice(0,8).map(p=><button className="generate-row" key={p.id} onClick={()=>addWeeklyFor(p)}><span>{p.glyph}</span><div><strong>{p.name}</strong><small>Generate next weekly boss</small></div><b>+</b></button>)}</aside>
      </div>
    </section>}

    {tab==="archive"&&<section className="page"><div className="archive-heading"><p className="kicker">ARCHIVE / NIGHTLY SAVE</p><h1>Reflection feeds the <em>next bearing.</em></h1></div>
      <div className="save-sheet"><label><span>WHAT BECAME EASIER?</span><textarea value={easier} onChange={e=>setEasier(e.target.value)}/></label><label><span>WHAT RESISTED?</span><textarea value={resisted} onChange={e=>setResisted(e.target.value)}/></label><label><span>WHERE NEXT?</span><textarea value={next} onChange={e=>setNext(e.target.value)}/></label><button className="ritual-button" onClick={saveArchive}>INSCRIBE SAVE</button></div>
    ...
    {tab==="build"&&<section className="page"><div className="build-hero"><DynamicInsignia state={state} selected={selectedPath} compact/><div><p className="kicker">BUILD / CURRENT SELF</p><h1>LVL {level}</h1><h2>{state.quests.filter(q=>q.done).length<4?"SEEKER":"BUILDER"}</h2><p>{state.xp.toLocaleString()} XP · MOMENTUM {state.momentum}</p></div></div>
      ...
      <section><div className="section-rule"><span>II</span><b>STORAGE MODE</b><i/></div><div className="storage-card"><strong>{process.env.NEXT_PUBLIC_SUPABASE_URL?"SUPABASE READY":"LOCAL-FIRST"}</strong><p>{process.env.NEXT_PUBLIC_SUPABASE_URL?"Cloud adapter is configured. Sign-in will activate per-user state.":"Your data saves in this browser. Add Supabase environment variables to switch the same state model to cloud persistence."}</p></div></section></div>
    </section>}
